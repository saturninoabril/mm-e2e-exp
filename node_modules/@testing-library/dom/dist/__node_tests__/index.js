...
var dtl = _interopRequireWildcard(require("../"));

test('works without a global dom', async () => {
  const container = new _jsdom.JSDOM(`
    <html>
      <body>
        <form id="login-form">
  ...
  const submittedDataPre = await dtl.findByTestId(container, 'submitted-data', {}, {
    container
  });
...
test('works without a browser context on a dom node (JSDOM Fragment)', () => {
  const container = _jsdom.JSDOM.fragment(`
    <html>
      <body>
        <form id="login-form">
          <label for="username">Username</label>
